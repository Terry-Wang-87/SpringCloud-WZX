...
import com.springboot.cloud.sysadmin.organization.exception.OrganizationErrorType;
...
public class GroupService extends ServiceImpl<GroupMapper, Group> implements IGroupService {
    ...
    public boolean delete(String id) {
    	Group tempG = this.getById(id);
    	if(null == tempG) throw new BaseException(OrganizationErrorType.ORG_NOT_FOUND);
    	String pId = tempG.getParentId();
    	QueryWrapper<Group> queryWrapper = new QueryWrapper<>();
    	queryWrapper.eq("parent_id", pId);
    	List result = this.list(queryWrapper);
    	if(0<result.size()) {
			throw new BaseException(OrganizationErrorType.ORG_HAS_CHILD);
		}else {
    ...
    public boolean update(Group group) {
    	...
    	if(StringUtil.isBlank(id)) throw new BaseException(OrganizationErrorType.ORG_PARAM_ERROR);
    ...
    public List<Group> query(GroupQueryParam groupQueryParam) {
        ...
        queryWrapper.like("name", groupQueryParam.getName());
        queryWrapper.eq("type", groupQueryParam.getType());
        queryWrapper.eq("code", groupQueryParam.getCode());
